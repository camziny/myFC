...
const WorldCupTeamsShowPage = (props) => {
  const { id } = useParams();
  ...
  const getWorldCupTeam = async () => {
    try {
      const response = await fetch(`/api/v1/worldCupTeams/${id}?pageNumber=${pageNumber}`);
      if (!response.ok) {
...
export default WorldCupTeamsShowPage;
